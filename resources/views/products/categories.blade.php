					...
					<div class="col-lg-3">
						<div class="product-box">
							<div class="product-box-img">
								@if(!empty($category->image) && count($category->image) > 0)
								<img src="{{ asset($category->image[0]) }}" class="img-fluid" alt="img">
								@else
								<img src="{{ asset('/uploads/products/no-image-icon-6.png') }}" class="img-fluid" alt="static-img">
								@endif
							</div>
							<div class="product-box-ctn">
								<div class="name-count" style="display:flex; justify-content:space-evenly;">
									<h4>{{$category->title}}</h4>
									<h4>{{ $category->products->count() }}</h4>
								</div>
								<a href="{{ route('categories.products', ['slug' => $category->slug]) }}" class="rm-btn-sm">View Details</a>
							</div>
						</div>
					</div>
					@endforeach
